...
import { variants, options, customComponentOptions, sizes, ThemeInterface, OptionProps } from './SingleSelect.stories.tsx';
...
If you want to use custom component options, pass the array of `{label:'' , value:''}` in the options list like below. `componentContent` to make component searchable
...
Provide `optionAutoWidth` flag true to take max content width for options modal.
